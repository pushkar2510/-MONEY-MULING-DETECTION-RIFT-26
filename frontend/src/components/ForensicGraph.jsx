...
export default function ForensicGraph({ elements }) {
  ...
  const stylesheet = [
    {
      // BEAUTIFUL FLOATING SAFE NODES
      selector: 'node',
      style: {
        'background-color': '#38bdf8', // Azure-400
        'border-color': '#0284c7', // Azure-700
        'border-width': 3,
        'width': 35, // MASSIVE SIZE
        'height': 35,
        'label': 'data(id)',
    ...
    {
      // HUGE FLOATING FRAUD NODES
      selector: 'node[?is_suspicious]',
      style: {
        'background-color': '#ff2a4d', 
        'border-color': '#991b1b',
        'border-width': 4,
        'width': 60, // EXTREMELY LARGE
        'height': 60,
        'color': '#ff2a4d',
        'font-size': '16px',
        // HEAVY RED GLOW FLOATING EFFECT
        'shadow-blur': 25,
        'shadow-color': '#ff2a4d',
        'shadow-opacity': 0.8,
        'shadow-offset-y': 8,
    ...
    {
      // THICK VISIBLE SAFE EDGES
      selector: 'edge',
      style: {
        'width': 2.5,
        'line-color': '#7dd3fc', // Azure-300
        'opacity': 0.8,
        'curve-style': 'bezier',
        'target-arrow-shape': 'triangle',
  ...
  // LOGIC TO MANAGE MASSIVE SPREAD AND PREVENT OVERLAP
  const isMassive = elements && elements.length > 2000;
  ...
  return (
    <div className="w-full h-full relative">
      <CytoscapeComponent
        ...
        minZoom={0.2} 
        maxZoom={3}
        // DEFAULT DRAG CAPABILITIES ENABLED
        userZoomingEnabled={true}
        userPanningEnabled={true}
        boxSelectionEnabled={true}
        autoungrabify={false} // Allows dragging nodes!
        ...
        layout={isMassive ? {
          ...
          spacingFactor: 4, // PUSHES 10K NODES MASSIVELY APART
          animate: false
        } : {
          name: 'cose',
          idealEdgeLength: 200, // VERY LONG EDGES TO SPREAD GRAPH
          nodeOverlap: 50,
          refresh: 20,
          fit: true,
          padding: 80,
          randomize: true,
          componentSpacing: 300, // MASSIVE SPACING BETWEEN RINGS
          nodeRepulsion: 2000000, // Nodes violently repel each other
          edgeElasticity: 50,
          nestingFactor: 5,
          animate: true,
        ...
        textureOnViewport={isMassive}
        hideEdgesOnPan={isMassive}
      ...
      {selectedNode && (
        <div className="absolute top-6 left-6 bg-white border-2 border-azure-200 p-6 rounded-2xl z-10 shadow-2xl min-w-[300px]">
          <h4 className="text-azure-600 mb-2 text-sm tracking-widest uppercase font-black">Node Identity</h4>
          <p className="font-mono font-bold text-2xl text-slate-900 mb-4 break-all">{selectedNode.id}</p>
          <div className="h-[2px] w-full bg-azure-100 mb-4"></div>
          
          <div className="flex justify-between items-end mb-4">
            <p className="text-base text-slate-600 font-bold">Threat Score</p>
            <p className={`text-4xl font-black ${selectedNode.score >= 50 ? "text-danger" : "text-azure-600"}`}>
              {selectedNode.score.toFixed(1)}
            </p>
          </div>
          
          {selectedNode.ring_id && (
            <div className="mt-6 bg-red-50 border border-red-200 p-4 rounded-xl flex flex-col gap-1">
              <span className="text-sm text-danger font-black tracking-wider uppercase">Associated Ring</span>
              <span className="font-mono text-slate-900 text-lg font-bold">{selectedNode.ring_id}</span>
            </div>
          )}
        </div>
      )}
